...
GAME_OVER_WON::GAME_OVER_WON()
{
	...
	m_sprite[1].setTexture(*(FileManager::getInstance().getScreen(GO_WON)));
...
void GAME_OVER_WON::add(std::string name, std::unique_ptr<Command>c)
{
	m_options.emplace_back(option(name, std::move(c)));
}
//--------------------------------------------------------
//MANAGE THE SCREEN OF G.O IF YOU WON
void GAME_OVER_WON::execute(sf::RenderWindow &window)
{
	sf::Event event;
	sf::Vector2u size;
	...
	window.create(sf::VideoMode(WINDOW_LENGTH, WINDOW_HEIGHT + 50), "NARCOS - go_won");
	...
	while (window.isOpen())
	{
		...
		while (window.pollEvent(event))
		{
			...
			auto inc = sf::Mouse::getPosition(window);

			for (int i = 0; i < m_sprites.size(); i++)
			{
				if (m_sprites[i].getGlobalBounds().contains(sf::Vector2f(inc)))
				{
